...
- **Smart Parallelization**: Automatically detects when process spawning overhead dominates 
  computation time and switches to sequential processing (e.g., for small datasets <2000 samples)
...
- **Platform-Aware Optimization**: Fast 'forkserver' on Linux/HPC, 'spawn' on Windows; 
  C engine (Linux) vs Python engine (Windows) for CSV reading
- **Single User Prompt**: Hardware selection asked once, used for both training and prediction

Main Features:
--------------
1. **Smart Parallel Training**: Automatically uses sequential or parallel based on workload size
...
Technical Details:
------------------
- Multiprocessing start method: 'forkserver' (Linux/HPC, faster) or 'spawn' (Windows, CUDA-safe)
- Smart parallelization: Compares overhead vs speedup, uses sequential for small datasets
...
- Overhead threshold: ~25s per process spawn; sequential used when overhead dominates
...
def parallel_cross_validation_fold(args):
    ...
    try:
        # Setup device (no delay needed with proper process pooling)
                    ...
                    for opt_params in opt_param_combos:
                        for lr in [opt_params.get('lr', 0.01)]:
                            for epochs in epoch_options:
                                ...
                                # Smart parallel decision: Only parallelize if workload is large enough
                                # Process spawning overhead (~20-30s per process) dominates small workloads
                                # Estimate: If single-fold training < 60s, sequential is faster
                                estimated_fold_time = size * 0.03  # Rough estimate: 0.03s per sample
                                process_overhead = 25  # seconds per spawned process
                                
                                if use_parallel and worker_type == 'GPU':
                                    # For multi-GPU: only parallelize if estimated speedup > overhead
                                    num_folds = 5
                                    sequential_time = estimated_fold_time * num_folds
                                    parallel_time = estimated_fold_time + (process_overhead * num_folds)
                                    
                                    if parallel_time >= sequential_time:
                                        logger.info(f"Workload too small ({estimated_fold_time:.1f}s/fold). Using sequential processing to avoid {process_overhead}s overhead per process.")
                                        use_parallel = False
                                ...
                                if use_parallel:
                                    ...
                                    try:
                                        ...
                                        # Use 'forkserver' on Linux for faster process creation on HPC
                                        # Use 'spawn' on Windows (required for CUDA compatibility)
                                        import platform
                                        if platform.system() == 'Windows':
                                            mp_context = mp.get_context('spawn')
                                        else:
                                            # Linux/HPC: 'forkserver' is much faster than 'spawn' (avoids re-importing)
                                            mp_context = mp.get_context('forkserver')
                                            logger.info("Using 'forkserver' multiprocessing context for faster HPC performance")
